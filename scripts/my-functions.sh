...
#  V2.0   20/12/2014  VK3MEL  Moved sounds to a unique directory, so they can be identified.
#
#  Used by vk3_irlp_wx script. 
#  I should really clean this up and lose the stuff we don't need.
#
#
##############################################################################
#
...
# custom audio file location
  CustomAudioDir="custom/vk3_irlp_wx"
...
function sendtone () {
   my_key
   usleep 500000
   $BIN/play $AUDIO/$CustomAudioDir/sound-confirm.wav     # contains 'beep beep' or whatever
   usleep 500000
   my_unkey
}
...
function senderror () {
  if [ "$1" != "notx" ]; then
    my_key
    usleep 500000
    $BIN/play $AUDIO/$CustomAudioDir/sound-error.wav  # whatever you want for errors
    $BIN/play $AUDIO/error.wav               # add std error message
    usleep 500000
    my_unkey
   else
    $BIN/play $AUDIO/$CustomAudioDir/sound-error.wav
...
function sendcomplete () {
  my_key
  usleep 500000
  $BIN/play $AUDIO/$CustomAudioDir/sound-complete.wav
  usleep 500000
  my_unkey
}
...
function sendvoice () {
  ...
  case $TTSengine in
  swift)      case $2 in
                text)   /usr/local/bin/swift -m text -f $1 ;;
                ssml)   /usr/local/bin/swift -f $1 ;;
                quote)  /usr/local/bin/swift $1 ;;
                *)      /usr/local/bin/swift -m text -f $1 ;;
              esac ;;
...
# 3- From news_tx-barg
  3)       txtcol red blk blink
           echo "News transmission failed"
           txtcol def
           echo
           writelog "News transmission failed due to file download Failure." ;;

# 4- From news_download-barg, WIA news
